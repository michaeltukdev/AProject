<div>
    @if ($project)
        <div x-cloak x-data="{ modalOpen: false }" @openproject.window="modalOpen = true">
                    ...
                    <div @click.stop class="z-10 w-full max-w-2xl rounded-lg shadow-2xl bg-container">
                        ...
                        <div class="p-5 border-b text-text-secondary border-b-input-border">
                            ...
                            <div class="flex items-center gap-3 mt-8">
                                <p class="p-2 text-sm rounded-lg bg-input-border">{{ date('F jS Y', strtotime($project->start_date)) }}</p>
                                <p>@svg('heroicon-o-arrow-long-right', 'w-4 h-4')</p>
                                <p class="p-2 text-sm rounded-lg bg-input-border">{{ date('F jS Y', strtotime($project->end_date)) }}</p>
                            </div>
                        </div>

                        <div class="flex items-center justify-between p-5">
                            <p class="text-sm text-text-secondary">Current phase: 
                                <span style="color: @switch($project->phase)
                                    @case('design')
                                        #3498db;
                                        @break
                                    @case('development')
                            ...
                            @if($project->user_id === auth()->id())
                                <button wire:click="delete({{ $project->id }})" class="p-2 text-sm rounded-lg bg-input-border">Delete</button>
                            @endif
                        </div>
                    </div>
                </div>
